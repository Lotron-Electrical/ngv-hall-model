...
let r = JSON.parse(await ev(`(()=>{const I=ngv.game.install;let bars=0,lit=0;
...
r = JSON.parse(await ev(`(()=>{const I=ngv.game.install,F=I.fx;const M=new T.Matrix4();F.cover.getMatrixAt(0,M);
...
r = JSON.parse(await ev(`(()=>{const F=ngv.game.install.fx;const g=ngv.game.install;const rd=()=>[...F.emit.instanceColor.array.slice(0,3)].map(v=>+v.toFixed(3));
...
r = JSON.parse(await ev(`(()=>{const I=ngv.game.install;I.setGuide(false);const rd=()=>[...I.fx.emit.instanceColor.array.slice(0,3)].map(v=>+v.toFixed(3));
...
r = await ev(`document.querySelector('#guide').textContent + '|' + ngv.game.install.guide`);
say(r === 'Guide on|true', `clicking the button turns the guide back on (${r})`);

// ---- screenshots
const N1 = JSON.parse(await ev(`(()=>{const g=ngv.game;const p=g.hallToWorld(7.71,3.82,g.world.floorY);return JSON.stringify([p.x,p.y,p.z]);})()`));
...
await ev(`(()=>{const g=ngv.game,I=g.install;I.fitted.clear();for(let i=0;i<I.slots.length;i++){I.fx.show(i,false);I.guides.show(i,true);}I.setGuide(true);
...
await ev(`(()=>{const I=ngv.game.install;const run=I.runs.find(r=>r.column==='N1');for(const s of run.slots.slice(0,3))I.fit(s,{carry:null});I.update(1/(1.2*2));})()`);
...
await ev(`(()=>{const g=ngv.game,I=g.install;I.setGuide(false);const run=I.runs.find(r=>r.column==='N1');const s=run.slots[1];
...
await ev(`(()=>{const I=ngv.game.install;I.setGuide(true);for(const s of I.slots)if(!I.fitted.has(s.id))I.fit(s,{carry:null});})()`);
await sleep(600);
const f0 = await ev(`ngv.game.dbg.frames`); const t0 = Date.now();
await sleep(2000);
const f1 = await ev(`ngv.game.dbg.frames`); const el = (Date.now() - t0) / 1000;
const fps = (f1 - f0) / el;
const fitted = await ev(`ngv.game.install.counts().fitted`);
