...
function page(){
 ...
 const cards=secs.map(s=>{ const d=s.rounds[s.rounds.length-1], sent=s.rounds.slice(0,-1);
  ...
  ${(T[s.key]||[]).length?`<div class="tx"><b>Text</b> <small>| before the word that glints, / for a second row, *...* for a row in stained glass</small>${T[s.key].map((l,idx)=>`<div class="line"><input autocomplete="off" data-key="${s.key}" data-idx="${idx}" value="${esc(l[0]).replace(/"/g,'&quot;')}"><span>${l[2]?esc(l[1]+' to '+l[2]+' s'):''} <i class="st" id="tx-${s.key}-${idx}"></i></span></div>`).join('')}</div>`:''}
