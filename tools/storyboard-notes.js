...
function page(){
 ...
 const cards=secs.map(s=>{ const d=s.rounds[s.rounds.length-1], sent=s.rounds.slice(0,-1);
  ...
  ${(T[s.key]||[]).length?`<div class="tx"><b>Text</b> <small>| before the word that glints, / for a new row, *...* for a row in stained glass, ~ for a row that comes in a word at a time</small>${T[s.key].map((l,idx)=>`<div class="line"><input autocomplete="off" data-key="${s.key}" data-idx="${idx}" value="${esc(l[0]).replace(/"/g,'&quot;')}"><span>${l[2]?esc(l[1]+' to '+l[2]+' s'):''} <i class="st" id="tx-${s.key}-${idx}"></i></span></div>`).join('')}</div>`:''}
...
.tx{margin:0 0 10px;border:1px solid #3a3320;border-radius:6px;padding:8px 10px;background:#1a1810}.tx b{font-size:13px;color:#dc9}.tx small{color:#887;font-size:12px}.line{display:flex;gap:8px;align-items:center;margin-top:6px}.line input{flex:1;background:#0d0d0d;color:#fff;border:1px solid #554;border-radius:6px;padding:8px 10px;font:inherit;min-width:0}.line input:focus{outline:none;border-color:#dc9}.line span{color:#887;font-size:12px;white-space:nowrap}.line i{font-style:normal}
