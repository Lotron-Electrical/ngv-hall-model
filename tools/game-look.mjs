...
await ev(`(()=>{const g=ngv.game, L=g.lift.pos, P=g.player; P.pos.set(L.x+3.5,g.world.floorY,L.z+2.5); P.yaw=Math.atan2(-(L.x-P.pos.x),-(L.z-P.pos.z)); P.pitch=-0.1;})()`); await sleep(400); await shot('lift');
// (2) raise the lift to 6 m and look from further
await ev(`(()=>{const g=ngv.game; g.lift.height=6; g.lift.refresh(); const L=g.lift.pos, P=g.player; P.pos.set(L.x+7,g.world.floorY,L.z+5); P.yaw=Math.atan2(-(L.x-P.pos.x),-(L.z-P.pos.z)); P.pitch=0.35;})()`); await sleep(400); await shot('lift-up');
// (3) the doors: stand 6 m off in the corridor looking at them, then step up to 2 m
await ev(`(()=>{const g=ngv.game; g.lift.height=0; g.lift.refresh(); const {hallToWorld}=g; })()`);
await ev(`(()=>{const g=ngv.game, D=g.world.doorCentre, P=g.player; const u=g.world.HALL||null; P.pos.copy(D); P.pos.x+=6*0.975681; P.pos.z+=6*0.219196; P.yaw=Math.atan2(-(D.x-P.pos.x),-(D.z-P.pos.z)); P.pitch=0;})()`); await sleep(1500); await shot('doors-shut');
console.log('shut?', await ev('ngv.game.world.doorsShut'));
await ev(`(()=>{const g=ngv.game, D=g.world.doorCentre, P=g.player; P.pos.copy(D); P.pos.x+=2.2*0.975681; P.pos.z+=2.2*0.219196;})()`); await sleep(1500); await shot('doors-open');
console.log('shut after approach?', await ev('ngv.game.world.doorsShut'));
// (4) the small hours: 04:20, from the hall floor looking down the hall
await ev(`(()=>{const g=ngv.game; g.clock.minute=28.33*60; const P=g.player; const p=g.world.doorCentre.clone(); P.pos.set(p.x-10*0.975681,g.world.floorY,p.z-10*0.219196); P.yaw=Math.PI/2+0.2; P.pitch=0.1;})()`); await sleep(2500); await shot('late');
console.log('fx level', await ev('ngv.game.fx.level'), 'hud', await ev(`document.querySelector('#stats').innerText.replace(/\n/g,' | ')`));
