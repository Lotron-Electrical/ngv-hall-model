...
#   python tools/chain_pixel.py <chain.json> <frame> u=48.056|h=9.4 px,py [px,py ...]
...
NV = HU if axis == 'u' else np.array([0, 1.0, 0]); off = 0.0 if axis == 'u' else O[1]
...
for pp in pix:
    ...
    num = val + off - ((C - O) @ NV if axis == 'u' else C[1]); den = D @ NV
