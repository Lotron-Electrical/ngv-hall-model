...
const act=async()=>{ await ev('ngv.game.player.actionQueued=true'); await sleep(150); };
...
await shot('controls');
await ev(`ngv.game.player.keys.add('KeyW')`); await sleep(600); console.log('4 ramp 0.6s speed', (await st()).speed); await sleep(2400); console.log('   3s:', JSON.stringify(await st()));
await ev(`ngv.game.player.keys.add('KeyD')`); await sleep(2500); console.log('5 steering:', JSON.stringify(await st()));
await ev(`ngv.game.player.keys.delete('KeyD');ngv.game.player.keys.delete('KeyW')`); await sleep(1500); console.log('6 released:', JSON.stringify(await st()));
await key(['KeyA'],1500); console.log('7 steer w/o drive (yaw must not change):', JSON.stringify(await st()));
await ev(`ngv.game.player.liftUp=true`); await sleep(3000); await ev(`ngv.game.player.liftUp=false`); console.log('8 raised:', JSON.stringify(await st()));
await key(['KeyW'],3000); console.log('9 creep at height:', JSON.stringify(await st()));
await shot('raised');
await ev(`ngv.game.player.liftDown=true`); await sleep(3500); await ev(`ngv.game.player.liftDown=false`);
...
await act(); await sleep(120); console.log('   mid-climb:', await prompt()); c=await climb(); console.log('   climb down:', JSON.stringify(c), c.eyeMin>1.6&&c.gateMax>1.5&&c.ticks<120&&!c.clips.length?'ok':'FAIL climb down'); console.log('12 got off:', await prompt(), JSON.stringify(await st()), 'eye', await ev('ngv.game.player.eye'));
await ev(`(()=>{const g=ngv.game,L=g.lift;const P=g.player;P.pos.copy(L.pos.clone().add(new P.pos.constructor(3,0,3)));P.pos.y=g.world.floorY;P.yaw=Math.atan2(-(L.pos.x-P.pos.x),-(L.pos.z-P.pos.z));P.pitch=0.1;})()`); await sleep(400); await shot('outside');
