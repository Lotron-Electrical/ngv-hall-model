...
const place=async(u,d,lookU,lookD,pitch=0)=>ev(`(()=>{const g=ngv.game,P=g.player; P.pos.copy(g.hallToWorld(${u},${d},g.world.floorY)); const T=g.hallToWorld(${lookU},${lookD},g.world.floorY+1.6); P.yaw=Math.atan2(-(T.x-P.pos.x),-(T.z-P.pos.z)); P.pitch=${pitch};})()`);
// lift in the hall at 6 m, seen from 9 m
await ev(`(()=>{const g=ngv.game; g.lift.pos.copy(g.hallToWorld(40,7.5,g.world.floorY)); g.lift.height=6; g.lift.refresh();})()`);
await place(31,7.5,40,7.5,0.35); await sleep(400); await shot('lift-hall');
await ev(`(()=>{const g=ngv.game; g.lift.height=0; g.lift.refresh();})()`); await place(36,7.5,40,7.5,-0.05); await sleep(400); await shot('lift-down');
// the doors: from the hall side 5 m off
await place(44,7.5,48.9,7.5); await sleep(2000); await shot('door-hall'); console.log('shut at 5m', await ev('ngv.game.world.doorsShut'));
...
console.log('after walk u=', await ev(`(()=>{const g=ngv.game,P=g.player; const v=P.pos.clone().sub(g.world.HALL?g.world.HALL.origin:new g.player.pos.constructor(-54.907447,-1.43545,3.040286)); return (v.x*0.975681+v.z*0.219196).toFixed(2);})()`));
